...
export async function POST(request) {
  ...
  try {
    ...
    const payload = normalizeLeadPayload(formData, {
      defaultFormType: formData?.project ? "PROJECT_FORM" : "CALLBACK_FORM",
    });
    locale = payload.locale || "en";

    const requiresProject = payload.formType === "PROJECT_FORM";
    const requiresEmail = payload.formType !== "CALLBACK_FORM";
    const validationMessage = validateLeadPayload(payload, {
      requireProject: requiresProject,
      requireEmail: requiresEmail,
    });
