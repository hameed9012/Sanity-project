...
function sanityToLand(p: any) {
  ...
  return {
    ...
    type: (p.type || project?.type || "residential").toLowerCase(),
    status: "available",
    title: { en: project?.name || p.name || slug, ar: p.nameAr || project?.name || slug },
    subtitle: { en: project?.location || p.location || "", ar: p.locationAr || project?.location || "" },
    description: { en: intro?.paragraphs?.[0] || intro?.description || "", ar: "" },
    area: { en: project?.units || p.unitType || "", ar: "" },
    price: { en: project?.startingPrice || p.startingPrice || "", ar: "" },
    completion: { en: project?.completionDate || "", ar: project?.completionDate || "" },
    developer: project?.developer || p.developer || "",
    gallery: Array.isArray(gallery?.slides)
      ? gallery.slides.map((s: any) => s?.url || s).filter(Boolean)
      : (p.image ? [p.image] : []),
    brochureUrl: intro?.brochures?.[0]?.url || "",
    masterplanUrl: "",
    details: { en: [], ar: [] },
    features: {
      en: Array.isArray(amenities?.amenities)
        ? amenities.amenities.map((a: any) => a?.label || "").filter(Boolean) : [],
      ar: [],
    },
    nearby: [],
  };
        ...
        <div className="land-main">
          ...
          <div>
            ...
            <div className="land-ctas">
              ...
              <Link href="/contact-us" className="land-cta-wa">
