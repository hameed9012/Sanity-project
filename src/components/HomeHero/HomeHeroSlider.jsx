...
export default function LuxuryHeroSlider() {
  ...
  const { allProjects: sanityProjects } = useAllProjects();
  ...
  // ✅ Build projects list from Sanity
  const projects = useMemo(() => {
    if (!sanityProjects?.length) return [];
            ...
            <form onSubmit={handleSearchSubmit} className={styles.searchBar}>
              <div className={styles.searchInputWrapper}>
                ...
                <input type="text" className={styles.searchInput} placeholder={searchPlaceholder}
                  value={searchQuery} onChange={handleInputChange} aria-label="Search properties" />
              </div>
