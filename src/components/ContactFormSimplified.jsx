...
export default function ContactFormSimplified({
  defaultTab = "BUY_PROPERTY",
  hideTabs = false, // Option to hide tabs if you want single form
}) {
  ...
  // ✅ BUY PROPERTY form data (SIMPLIFIED)
  const [buyFormData, setBuyFormData] = useState({
    firstName: "",
    lastName: "",
    phone: "",
    email: "",
    message: "", // Added message field for general contact
    contactMethod: "phone",
    agreePrivacy: false,
    agreeNews: false,
  });
  ...
  const trackFormStartedIfNeeded = (fieldName) => {
    ...
    fbTrack("custom", "FormStarted", {
      form: activeTab === "BUY_PROPERTY" ? "ContactForm" : "CareersForm",
      field: fieldName,
      locale,
    });
  };

  // ✅ Handle Buy Property form changes (SIMPLIFIED)
  ...
  const fullPhone = `+${selectedCountry.dial}${
    activeTab === "BUY_PROPERTY" ? buyFormData.phone : careersFormData.phone
  }`;

  // ✅ Handle Contact form submission (SIMPLIFIED - for general contact)
  const handleContactSubmit = async (e) => {
    ...
    try {
      const response = await fetch("/api/contact-general", {
        ...
        body: JSON.stringify({
          firstName: buyFormData.firstName,
          lastName: buyFormData.lastName,
          phone: fullPhone,
          email: buyFormData.email,
          message: buyFormData.message || "General contact inquiry",
      ...
      if (result.success) {
        ...
        // ✅ GA4 conversion event (only on real success)
        ga.event({
          action: "submit_lead",
          category: "conversion",
          label: "contact_form",
